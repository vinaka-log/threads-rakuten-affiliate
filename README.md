...
- スケジュール: **毎日10投稿 JST**（共感つぶやき7 : 商品紹介2 : 攻略1）
  - **共働きリアル苦悩 7本**: 売り込み前の生活実感・あるある吐露（閲覧が伸びやすい）
  - 商品紹介 2本: 夕方以降のみ。本投稿リンクなし + 自分リプ（冒頭 `#PR` + アフィリエイトURL）
  - 攻略・保存ネタ 1本: 18:00
  - ランキングダイジェストは一旦停止（共感優先）
  - 17:00 は再利用キュー優先（伸びた投稿の再投下）
...
枠の種別は `config.py` の `ITEM_SLOTS` / `DIGEST_SLOTS` / `VALUE_SLOTS` / `STRUGGLE_SLOTS` で変更できます。CLI では `--item` / `--value` / `--digest` で種別を強制、`--value-id` や `--digest-format`（top3 / quiz / sleeper）で内容を指定できます。
...
- **slot 0 / 1 / 2 / 3 / 4 / 8 / 9（7本）**: **30代共働きのリアル苦悩**（35本ローテ）。夜ご飯・寄り道・消耗品など生活実感の吐露。返信を取りにいく
- **slot 5（18:00）**: セール攻略・レビューの読み方など保存ネタ（36本ローテ）
