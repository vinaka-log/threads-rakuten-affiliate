...
- スケジュール: **毎日10投稿 JST**（repo は public のため Actions 枠は消費しない）
  ...
  - **商品紹介 2本（16:00 / 22:00）**: 本投稿は買い足しあるある＋問い（商品名・画像なし）。自分リプ1=うちのストックメモ、リプ2=URL＋`※PR`
  - ランキングダイジェストは停止
...
1. **本投稿** … URLなし・商品名なし・商品画像なし（目安〜120字）。買い足しあるある → 短い共感 → 問い。価格・レビュー・PRなし
2. **自分リプ1** … `うちのストックはこれ。` + 商品名 + 注意点（必要なら今日のポイント日だけ）。リンク・PRなし
3. **自分リプ2** … URL + 末尾 `※PR` のみ

収納ガジェットの誤マッチは `exclude_name_hints` / `BLOCK_NAME_HINTS` で除外。
