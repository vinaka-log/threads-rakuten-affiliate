...
1. **本投稿** … URLなし（商品画像添付可）。構成は **痛み → 短い共感/困り1つ → 商品名 → リプ誘導1つ**（目安〜140字）。価格・レビュー・注意点は本投稿に出さない
2. **自分リプ** … 正直な注意点 → 価格・レビュー（順位は補足）→ 送料/ポイント短文 → `▼商品はこちら` + `affiliateUrl` → `※PR（アフィリエイトリンク）`。本編ベネフィットの繰り返しはしない
