...
- スケジュール: **毎日10投稿 JST**（共感5 : 雑談2 : 商品紹介3）+ **問いかけ雑談2本**（11:00 / 16:00）
  - **共働きリアル苦悩 5本**: 生活実感・あるある吐露
  - **どうでもいい雑談 2本**: テーマ無関係（中の人が人間に見えるためのブレ）
  - **問いかけ雑談 2本（追加）**: 伸びたアイス投稿と同型。季節 + 軽いトレンド/ニュース。商品・PRなし
...
├── .github/workflows/ask-chitchat-extra.yml  # 11:00 / 16:00 問いかけ雑談
├── config.py                 # ジャンル・品質フィルタ・枠・セール日程
├── data/posted.json          # 投稿済み台帳
├── data/reuse.json           # 価値投稿の再利用キュー
├── data/ask_chitchat_pool.json  # 問いかけ雑談プール
├── data/trend_seeds.json     # 軽いトレンド見出しキャッシュ
├── requirements.txt
└── src/
    ...
    ├── ask_chitchat.py       # 問いかけ雑談（季節+トレンド）
...
- 景品表示法（ステマ規制）により、アフィリエイトを含む投稿には **広告であることの明示** が必要です。本ツールはリンクリプのリンク直後に `※PR（アフィリエイトリンク）` を自動挿入します。
...
| 0 | 07:00 | **共働きリアル苦悩** |
| 1 | 08:00 | **どうでもいい雑談** |
| — | 11:00 | **問いかけ雑談**（追加・季節/トレンド） |
| 2 | 12:00 | **共働きリアル苦悩** |
| 3 | 15:00 | **どうでもいい雑談** |
| — | 16:00 | **問いかけ雑談**（追加・季節/トレンド） |
...
通常10枠に加え、`ask-chitchat-extra.yml` が **11:00 / 16:00** に問いかけ雑談を2本投下する（伸びた「夏といえばアイス」型）。季節ネタと軽いトレンド（スポーツ等のRSS）を `data/ask_chitchat_pool.json` から消費し、未使用が減ったら自動補充。商品・PRなし。
...
- **11:00 / 16:00（追加2本）**: **問いかけ雑談**。季節フック＋みんなのオススメ募集＋自分の答え（しろくま）。軽いトレンド/ニュース見出しも混ぜる（`ask_chitchat.py` / NHKスポーツ等RSS）。重い事件・政治は除外
