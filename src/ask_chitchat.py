...
_SEASONAL_POLLS: dict[int, Sequence[Tuple[str, Tuple[str, str, str, str]]]] = {
    ...
    8: (
        ...
        ("夏の終わり、いちばん名残惜しいのは？", ("花火", "かき氷", "長い昼", "特になし")),
        ("残暑の外出、どうしてる？", ("朝イチで出る", "夕方まで待つ", "ほぼ家", "気にしない")),
    ),
    9: (
        ("秋といえば食欲、何が食べたい？", ("焼き芋", "さんま", "きのこ", "その他")),
        ("残暑でもアイス食べる？", ("まだ食べる", "もう卒業", "たまに", "季節無関係")),
        ("敬老の日のお土産、何が無難？", ("お茶セット", "お菓子", "花", "まだ決めてない")),
        ("秋の夜長、何時に寝る？", ("22時前", "24時前後", "深夜型", "日による")),
        ("運動会シーズン、どう関わってる？", ("応援する", "写真係", "今年は遠い", "関係ない")),
        ("スーパーSALE、狙うものある？", ("日用品まとめ", "ベビーグッズ", "特になし", "まだ見てない")),
    ),
    10: (
...
_EVERGREEN_POLLS: Sequence[Tuple[str, Tuple[str, str, str, str]]] = (
    ...
    # 0〜2歳あるある（商品・PRなし）
    ("寝かしつけ、平均どれくらい？", ("15分以内", "30分前後", "1時間超え", "日による")),
    ("夜中のおむつ替え、誰が多い？", ("自分多め", "パートナー多め", "半々", "まだ先")),
    ("お出かけバッグ、何が一番重い？", ("おむつ替え", "飲み物", "おもちゃ", "全部重い")),
    ("急な雨、ベビーカーどうする？", ("カバーある", "走って帰る", "屋内退避", "まだ無縁")),
    ("子ども服の片付け、どうしてる？", ("床置きがち", "ハンガー派", "カゴ投げ", "パートナー任せ")),
    ("手形足形、もう撮った？", ("撮った", "今月やる", "先延ばし中", "興味薄い")),
    ("おしりふき、切れやすい？", ("すぐ切れる", "まあまあ持つ", "余りがち", "まだ使わない")),
    ("抱っこ移動、しんどい派？", ("毎日しんどい", "たまに", "慣れた", "まだ無縁")),
    ("離乳食、いまどの段階？", ("初期", "中期〜後期", "幼児食", "これから")),
    ("保育園の送り、朝の勝負は？", ("時間との勝負", "割と余裕", "日による", "まだ通ってない")),
)
