...
def register_value_post(
    *,
    ...
    """価値投稿を再利用キューに登録/更新。雑談・アンケートは登録しない。"""
    from value_posts import is_oneshot_value_id

    if is_oneshot_value_id(value_id):
        raise ValueError(f"oneshot value cannot enter reuse queue: {value_id}")
    _find(value_id)  # 存在確認
