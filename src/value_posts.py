...
アルゴリズム評価と信頼を積む。リンク・PR表記は一切入れない。

ペルソナ B（0〜2歳のベビーグッズ買い）向け:
  お出かけ・ねんね・収納・ポイント日の買い時が主語。
...
#   - 主語は「0〜2歳の買い足し担当」に寄せる（日用品総合・美容・ガジェットは扱わない）
_POOL: Sequence[ValuePost] = (
    ...
    ValuePost(
        ...
        "0〜2歳の買い足しをメモするしろくまです。\n"
        "・お出かけ・ねんね・収納の候補を毎日メモ\n"
        "・売れ筋と口コミの買い時を伝える\n"
        "・ポイント日やセールの目安も流す\n"
        "・紹介にはPR（アフィリエイトリンク）を含みます\n\n"
        "これから買うベビーグッズを楽天に寄せたい人向け。\n\n"
        "フォローのきっかけ、よかったら教えて",
    ),
)

# 0〜2歳育児のリアル（リンクなし・PRなし）。
# STRUGGLE_SLOTS 専用（現在は空でも、雑談枯渇時のフォールバックに使う）。
_STRUGGLE_POOL: Sequence[ValuePost] = (
    ValuePost(
        "struggle-night-diaper",
        "夜中のおむつ替え、\n"
        "床のおむつ山から探すのやめたい…\n\n"
        "暗がりでガサガサしてたら\n"
        "子どももこっちも起きる\n\n"
    ...
    ValuePost(
        "struggle-size-up",
        "おむつサイズ、まだいけると思った翌週にモレる\n\n"
        "粘った自分が悪いのは分かる。\n"
        "でも次の箱を開封するタイミング難しい\n\n"
        "サイズ上げ、何で判断してる？",
    ),
    ValuePost(
        "struggle-clothes-floor",
        "子ども服、床に増殖して朝戦場になるの\n"
        "うちだけ？\n\n"
        "ハンガー低いと自分で取れる説は聞くけど\n"
        "導入前がしんどい\n\n"
    ...
    ValuePost(
        "struggle-delivery-baby",
        "ベビーグッズの宅配、玄関がダンボールだらけ\n\n"
        "買うのは楽。開けるのと捨てるの誰がやる問題\n\n"
        "段ボール、すぐ畳めてる人尊敬する",
    ),
    ValuePost(
        "struggle-tiny-win-baby",
        "今日いちばんの勝利、\n"
        "おしりふき切れそうなのに先に買えたこと\n\n"
        "小さいけど、外出で慌てなくて済むやつ\n\n"
        "地味な勝ち報告、なんかある？",
